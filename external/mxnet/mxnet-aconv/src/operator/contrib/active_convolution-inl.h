      ...
      virtual void Backward(const OpContext &ctx,
        ...
        for (index_t n = 0; n < num_; ++n) {
          ...
          active_col2im_coord(s, col_buffer.dptr<DType>(),
            ...
            // print offset
            /*
            Tensor<xpu, 1, DType> offset_gpu = in_data[conv::kOffset].get_with_shape<xpu, 1, DType>(
                    Shape1(param_.kernel[0] * param_.kernel[1] * 2), s);
            Tensor<cpu, 1, DType> offset_cpu = NewTensor<cpu>(offset_gpu.shape_, DType(0));
            Copy(offset_cpu, offset_gpu, s);
            cout << "----------- offset -----------" << endl;
            for (index_t i = 0; i < param_.kernel[0]; ++i) {
                for (index_t j = 0; j < param_.kernel[1]; ++j) {
                    index_t index = (i * param_.kernel[1] + j) * 2;
                    cout << "(" << setw(15) << setprecision(8) << offset_cpu[index] << ", " << setw(15) << setprecision(8) << offset_cpu[index + 1] << ") | ";
                }
                cout << endl;
            }*/
            ...
            Tensor<xpu, 1, DType> doffset_gpu = in_grad[conv::kOffset].get_with_shape<xpu, 1, DType>(
                    Shape1(param_.kernel[0] * param_.kernel[1] * 2), s);
            Tensor<cpu, 1, DType> doffset_cpu = NewTensor<cpu>(doffset_gpu.shape_, DType(0));
            Copy(doffset_cpu, doffset_gpu, s);
            //cout << "----------- doffset -----------" << endl;
            for (index_t i = 0; i < param_.kernel[0]; ++i) {
                for (index_t j = 0; j < param_.kernel[1]; ++j) {
                    index_t index = (i * param_.kernel[1] + j) * 2;
            ...
            Copy(doffset_gpu, doffset_cpu, s);

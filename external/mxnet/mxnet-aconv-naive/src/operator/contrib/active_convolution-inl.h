 ...
 * \file active_convolution-inl.h
 * \brief
 * \ref: https://github.com/Yangqing/caffe/wiki/Convolution-in-Caffe:-a-memo
 * \ref: https://arxiv.org/abs/1703.06211
 * \author Yuwen Xiong, Haozhi Qi, Jifeng Dai
*/
#ifndef MXNET_OPERATOR_ACTIVE_CONVOLUTION_INL_H_
#define MXNET_OPERATOR_ACTIVE_CONVOLUTION_INL_H_
...
namespace mxnet {
  namespace op {
    ...
    namespace conv {
      enum ActiveConvolutionOpInputs { kData, kOffset, kWeight, kBias };
      enum ActiveConvolutionOpOutputs { kOut };
      enum ActiveConvolutionOpResource { kTempSpace };
    }

    struct ActiveConvolutionParam : public dmlc::Parameter<ActiveConvolutionParam> {
      TShape kernel;
      TShape stride;
      TShape dilate;
      TShape pad;
      uint32_t num_filter;
      uint32_t num_group;
      uint32_t num_active_group;
      uint64_t workspace;
      bool no_bias;
      dmlc::optional<int> layout;
      DMLC_DECLARE_PARAMETER(ActiveConvolutionParam) {
        ...
        DMLC_DECLARE_FIELD(num_active_group).set_default(1)
          .describe("Number of active group partitions.");
    ...
    class ActiveConvolutionOp : public Operator {
    public:
      explicit ActiveConvolutionOp(ActiveConvolutionParam p) {
        this->param_ = p;
      ...
      virtual void Forward(const OpContext &ctx,
        ...
        for (index_t n = 0; n < num_; ++n) {
          ...
          active_im2col(s, in_data[conv::kData].dptr<DType>() + n*input_dim_,
            in_data[conv::kOffset].dptr<DType>() + n*input_offset_dim_, in_data[conv::kData].shape_,
            col_buffer.shape_, param_.kernel, param_.pad, param_.stride, param_.dilate, param_.num_active_group,
      ...
      virtual void Backward(const OpContext &ctx,
        ...
        for (index_t n = 0; n < num_; ++n) {
          // in_grad[Data](col) = weight' * out_grad[Data]
          // (C1 * K * K) * (H2 * W2) = (C2 * (C1 * K * K))' * (C2 * (H2 * W2))
          Tensor<xpu, 3, DType> out_grad_3d = out_grad_4d[n];
          for (index_t g = 0; g < group_; ++g) {
            col_buffer_3d[g] = dot(weight_3d[g].T(), out_grad_3d[g]);
          }

          // gradient w.r.t. input coordinate data (doffset)
          // in_grad[Data](col), in_data[Data], in_data[Offset] -> in_grad[Offset]
          // col_buffer.shape_ = (C1 * K * K, H2, W2)
          active_col2im_coord(s, col_buffer.dptr<DType>(),
            in_data[conv::kData].dptr<DType>() + n*input_dim_, in_data[conv::kOffset].dptr<DType>(),
            in_grad[conv::kData].shape_, col_buffer.shape_,
            param_.kernel, param_.pad, param_.stride, param_.dilate, param_.num_active_group,
            in_grad[conv::kOffset].dptr<DType>() + n*input_offset_dim_,
            req[conv::kData]);
          // calculate normalized gradient (active convolution paper, section 2.5)
          Tensor<xpu, 3, DType> doffset = in_grad[conv::kOffset].get_with_shape<xpu, 3, DType>(
            Shape3(param_.kernel[0], param_.kernel[1], 2));
          for (index_t i = 0; i < param_.kernel[0]; ++i) {
            for (index_t j = 0; j < param_.kernel[1]; ++j) {
              DType Z = doffset[i][j][0] * doffset[i][j][0] + doffset[i][j][1] * doffset[i][j][1];
              Z = sqrt(Z);
              doffset[i][j][0] /= Z;
              doffset[i][j][1] /= Z;
            }
          ...
          /*
          // DEBUG
          Tensor<xpu, 1, DType> offset_1d_data = in_data[conv::kOffset].get_with_shape<xpu, 1, DType>(
                  Shape1(2 * param_.kernel[0] * param_.kernel[1]), s);
          Tensor<xpu, 1, DType> offset_1d_grad = in_grad[conv::kOffset].get_with_shape<xpu, 1, DType>(
                  Shape1(2 * param_.kernel[0] * param_.kernel[1]), s);
          Tensor<cpu, 1, DType> offset_1d_data_cpu = NewTensor<cpu, 1>(offset_1d_data.shape_, DType(0), true, s);
          Tensor<cpu, 1, DType> offset_1d_grad_cpu = NewTensor<cpu, 1>(offset_1d_grad.shape_, DType(0), true, s);
          Copy(offset_1d_data_cpu, offset_1d_data, s);
          Copy(offset_1d_grad_cpu, offset_1d_grad, s);
          std::cout << "[DEBUG] offset_1d_data " << offset_1d_data_cpu.dptr_[0] << std::endl;
          std::cout << "[DEBUG] offset_1d_grad " << offset_1d_grad_cpu.dptr_[0] << std::endl;
          */

          // gradient w.r.t. input data
          // in_grad[Data](col) -> in_grad[Data]
          // (C1 * K * K) * (H2 * W2) -> C1 * H1 * W1
          active_col2im(s, col_buffer.dptr<DType>(),
          ...
          // in_data[Data] -> in_data[Data](col)
          // C1 * (H1 * W1) -> (C1 * K * K) * (H2 * W2)
          active_im2col(s, in_data[conv::kData].dptr<DType>() + n*input_dim_,
            in_data[conv::kOffset].dptr<DType>() + n*input_offset_dim_, in_data[conv::kData].shape_,
            col_buffer.shape_, param_.kernel, param_.pad, param_.stride, param_.dilate, param_.num_active_group,
            col_buffer.dptr<DType>());

          // dweight = out_grad[Data] * in_data[Data](col)'
          // C2 * (C1 * K * K) = (C2 * (H2 * W2)) * ((C1 * K * K) * (H2 * W2))'
          for (index_t g = 0; g < group_; ++g) {
            if (0 == n) {
    ...
    private:
      void LayerSetUp(const TShape& ishape, const TShape& offset_shape, const TShape& oshape) {
        ...
        /*
        std::cout << "-----------" << std::endl;
        std::cout << "[DEBUG] ishape " << ishape << std::endl; // (1, 512, 48, 64, )
        std::cout << "[DEBUG] offset_shape " << offset_shape << std::endl; // (18, )
        std::cout << "[DEBUG] oshape " << oshape << std::endl; // (1, 512, 48, 64, )
        */
      }

    private:
      ActiveConvolutionParam param_;
    ...
    Operator* CreateOp(ActiveConvolutionParam param, int dtype,
      std::vector<TShape> *in_shape,
      std::vector<TShape> *out_shape,
      Context ctx);

#if DMLC_USE_CXX11
    class ActiveConvolutionProp : public OperatorProperty {
    public:
      ...
      bool InferShape(std::vector<TShape> *in_shape,
        ...
        if (param_.kernel.ndim() == 2) {
          ...
          // offset
          Shape<1> offsetshape = Shape1(2 * param_.kernel[0] * param_.kernel[1]);
          SHAPE_ASSIGN_CHECK(*in_shape, conv::kOffset, offsetshape);
          // weight
          ...
          CHECK_EQ(dshape[1] % param_.num_active_group, 0U) \
            << "input num_filter must divide active group size";
          ...
          CHECK_EQ(oshape[1] % param_.num_active_group, 0U) \
            << "output num_filter must divide active group size";
      ...
      OperatorProperty* Copy() const override {
        auto ptr = new ActiveConvolutionProp();
        ptr->param_ = param_;
        return ptr;
      }

      std::string TypeString() const override {
        return "_contrib_ActiveConvolution";
      }
